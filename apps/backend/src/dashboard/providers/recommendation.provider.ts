...
export class DashboardRecommendationProvider {
  ...
  private async getLandlordRecommendations(
    landlordId: string,
  ): Promise<RecommendationItem[]> {
    ...
    if (listingsCount === 0) {
      list.push({
        ...
        description:
          'ابدأ بنشر وحدتك السكنية أو الأسرة المتاحة للايجار وجذب المستأجرين بسهولة.',
  ...
  private async getTenantRecommendations(
    tenantId: string,
  ): Promise<RecommendationItem[]> {
    const list: RecommendationItem[] = [];
    ...
    if (alertsCount === 0) {
      list.push({
        ...
        description:
          'حدد محافظتك وميزانيتك ليصلك إشعار فوري فور إضافة أي شقة أو سرير يناسب رغبتك.',
