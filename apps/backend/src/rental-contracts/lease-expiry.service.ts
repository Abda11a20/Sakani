...
export class LeaseExpiryService {
  ...
  async handleExpiredContracts(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Found ${expiredContracts.length} expired contracts to process.`);
      ...
      for (const contract of expiredContracts) {
        try {
          if (contract.isAutoRenew) {
            ...
            this.logger.log(`Auto-renewing contract ${contract.contractNumber} to ${newEndDate}`);
            await this.contractsService.renewContract(contract.id, contract.landlordId, {
              newEndDate,
              createdByType: 'AUTO_RENEW',
            });
          } else {
            // Terminate naturally as expired
            this.logger.log(`Expiring contract ${contract.contractNumber} naturally.`);
            await this.prisma.$transaction(async (tx) => {
              await this.contractsService.terminateExpiredContract(contract.id, tx);
            });
          }
        } catch (err: any) {
          this.logger.error(`Failed to process lease expiry/renewal for contract ${contract.contractNumber}: ${err?.message || err}`);
        }
      }
    } catch (error: any) {
      this.logger.error(`Error querying expired rental contracts: ${error?.message || error}`);
    }
