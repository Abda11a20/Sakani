...
import { ContractStatus, PaymentCycle, TerminationReason, ContractCreatedBy, Prisma, RentalContract, NotificationType } from '@prisma/client';
...
export class RentalContractsService {
  ...
  private async generateContractNumber(tx: Prisma.TransactionClient): Promise<string> {
    const year = new Date().getFullYear();
    // Ensure the sequence exists dynamically
    await tx.$executeRawUnsafe('CREATE SEQUENCE IF NOT EXISTS contract_number_seq START 1;');
    const result = await tx.$queryRawUnsafe<any[]>("SELECT nextval('contract_number_seq')");
    const nextval = result[0]?.nextval;
    const seq = String(nextval || 1).padStart(6, '0');
    return `SKN-CNT-${year}-${seq}`;
  }

  async createContract(dto: CreateContractDto, tx: Prisma.TransactionClient): Promise<RentalContract> {
  ...
  async terminateContract(contractId: string, landlordId: string, dto: TerminateContractDto): Promise<RentalContract> {
  ...
  async renewContract(contractId: string, landlordId: string, dto: RenewContractDto): Promise<RentalContract> {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (old.status !== ContractStatus.active && old.status !== ContractStatus.expired) {
        throw new BadRequestException('لا يمكن تجديد هذا العقد لأن حالته غير قابلة للتجديد');
      }
      ...
      const newContract = await this.createContract({
        ...
        monthlyRent: dto.newMonthlyRent !== undefined ? dto.newMonthlyRent : old.monthlyRent,
        securityDeposit: old.securityDeposit,
        paymentCycle: old.paymentCycle,
        currency: old.currency,
        isAutoRenew: dto.isAutoRenew !== undefined ? dto.isAutoRenew : old.isAutoRenew,
        notes: dto.notes,
      }, tx);
  ...
  async getListingContractAnalytics(listingId: string, landlordId: string) {
    ...
    const totalRevenue = contracts.reduce((acc, c) => {
      ...
      const end = (c.actualCheckout ? new Date(c.actualCheckout) : new Date(c.endDate)).getTime();
  ...
  async terminateExpiredContract(contractId: string, tx: Prisma.TransactionClient): Promise<void> {
    const contract = await tx.rentalContract.findUnique({
      where: { id: contractId },
    });
