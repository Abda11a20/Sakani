...
function HistoryCard({ item, locale, onClick }: HistoryCardProps) {
  ...
  return (
    <div 
      onClick={onClick}
      className={`group bg-white rounded-3xl border border-slate-200 p-4 sm:p-5 hover:shadow-md hover:border-blue-200 transition-all hover:scale-[1.01] flex flex-col justify-between items-center text-center gap-3 relative ${onClick ? 'cursor-pointer' : ''}`}
    >
      ...
      <div className="shrink-0 mt-2">
        {coverUrl ? (
          ...
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-blue-50 flex items-center justify-center border-2 border-blue-100 shadow-sm">
            {isApartment ? (
...
function EmptyState({ locale }: { locale: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <div className="w-20 h-20 rounded-2xl bg-blue-50 flex items-center justify-center mb-4 border border-blue-100">
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <div className="relative flex-1 min-w-52">
              ...
              <input
                ...
                className="w-full text-sm border border-slate-200 rounded-xl ps-8 pe-4 py-2 bg-white text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 font-cairo"
              />
            </div>
            <button
              onClick={() => { setSort((s) => (s === "desc" ? "asc" : "desc")); setPage(1); }}
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 bg-white text-xs font-medium text-slate-600 hover:border-blue-300 hover:text-blue-600 transition-colors font-cairo"
            >
          ...
          <div className="p-1 sm:p-2 space-y-3 font-cairo">
              ...
              {selectedItem.listing.landlord && (
                <div className="bg-slate-50 p-2.5 rounded-lg border border-slate-100">
                  <div className="flex items-center gap-1 mb-1 text-slate-500">
                    ...
                    <span className="text-[10px] font-medium">{isRtl ? "المؤجر" : "Landlord"}</span>
                  </div>
                  <div className="flex flex-col gap-0.5">
                    <p className="font-bold text-xs">{selectedItem.listing.landlord.name}</p>
                    {selectedItem.listing.landlord.phone && (
                      <a href={`tel:${selectedItem.listing.landlord.phone}`} className="font-bold text-blue-600 hover:underline text-[10px] font-sans block" style={{ direction: "ltr", textAlign: isRtl ? "right" : "left" }}>
                        {selectedItem.listing.landlord.phone}
                      </a>
                    )}
                  </div>
            ...
            <div className="pt-2 border-t border-slate-200 flex items-center gap-2">
              <Link
                ...
                className="flex-1 text-center inline-flex justify-center items-center gap-1 text-xs font-semibold px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors"
              >
